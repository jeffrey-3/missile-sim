...
class Plot:
    def __init__(self, queue, limits):
        ...
        self.line_xy, = self.ax.plot([], [], [], color=(0, 0, 0))
        self.line_xz, = self.ax.plot([], [], [], color=(0, 0, 0))
        self.line_yz, = self.ax.plot([], [], [], color=(0, 0, 0))
        ...
        self.target_xy, = self.ax.plot([], [], [], color=(0, 0, 0),
            linestyle='none', marker='*', markersize=10)
        self.target_xz, = self.ax.plot([], [], [], color=(0, 0, 0),
            linestyle='none', marker='*', markersize=10)
        self.target_yz, = self.ax.plot([], [], [], color=(0, 0, 0),
            linestyle='none', marker='*', markersize=10)
    ...
    def update(self, frame):
        ...
        if data:
            ...
            self.line_xy.set_3d_properties([0] * len(self.x_data))
            self.line_xz.set_data(self.x_data, [self.limits] * len(self.x_data))
            self.line_xz.set_3d_properties(self.z_data)
            self.line_yz.set_data([self.limits] * len(self.x_data), self.y_data)
            self.line_yz.set_3d_properties(self.z_data)
            self.target.set_data([target_pos[0]], [target_pos[1]])
            self.target.set_3d_properties([target_pos[2]])
            self.target_xy.set_data([target_pos[0]], [target_pos[1]])
            self.target_xy.set_3d_properties([0])
            self.target_xz.set_data([target_pos[0]], [self.limits])
            self.target_xz.set_3d_properties([target_pos[2]])
            self.target_yz.set_data([self.limits], [target_pos[1]])
            self.target_yz.set_3d_properties([target_pos[2]])
            self.target_line.set_data([pos[0], target_pos[0]],
                [pos[1], target_pos[1]])
            self.target_line.set_3d_properties([pos[2], target_pos[2]])
        return (self.line, self.line_xy, self.line_xz, self.line_yz,
            self.x_axis, self.y_axis, self.z_axis, self.target,
            self.target_line, self.target_xy, self.target_xz, self.target_yz)
